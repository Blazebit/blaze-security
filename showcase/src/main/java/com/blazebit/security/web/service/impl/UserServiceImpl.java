...
public class UserServiceImpl implements UserService {
	...
	public void delete(User user) {
		...
		permissionManager
				.remove(permissionManager.getPermissions(reloadedUser));
		...
		return entityManager
				.createQuery(
						"select user from "
								...
								+ " user where user.username != 'superAdmin' and user.company.id='"
								+ company.getId() + "' order by user.id",
		...
		try {
			return entityManager.createQuery(
					"select u from "
							...
							+ username
							+ "'"
							+ (company != null ? " and u.company.id="
									+ company.getId() : ""), User.class)
					.getSingleResult();
