...
@ViewScoped
public class GroupUsersBean extends PermissionTreeHandlingBaseBean implements
		PermissionView, Serializable {
	...
	private void initPermissions() {
		...
		for (UserGroup group : parents) {
			groupNode = new DefaultTreeNode(new TreeNodeModel(group.getName(),
					ResourceType.USERGROUP, group), groupNode);
			groupNode.setExpanded(true);
			groupNode.setSelectable(false);
			List<Permission> groupPermissions = permissionManager
					.getPermissions(group);
			List<Permission> permissions = permissionHandlingUtils
					.filterPermissions(groupPermissions).get(0);
			List<Permission> dataPermissions = permissionHandlingUtils
					.filterPermissions(groupPermissions).get(1);
			selectedGroupPermissions.addAll(permissions);
			groupNode = getPermissionTree(permissions, dataPermissions);
		}
		selectedGroupPermissions = permissionHandlingUtils
				.getNormalizedPermissions(selectedGroupPermissions);
	}

	private void initUsers() {
		List<User> allUsers = userService.findUsers(userSession
				.getSelectedCompany());
	...
	private void processSelectedUsers() {
		...
		for (UserModel userModel : userList) {
			List<Permission> permissions = permissionManager
					.getPermissions(userModel.getUser());
			List<Permission> userPermissions = permissionHandlingUtils
					.filterPermissions(permissions).get(0);
			List<Permission> userDataPermissions = permissionHandlingUtils
					.filterPermissions(permissions).get(1);

			if (userModel.isSelected()) {
				// user is selected
				TreeNode currentUserNode = createCurrentUserNode(
						currentPermissionRoot, userModel.getUser(),
						userPermissions, userDataPermissions, true);
				TreeNode newUserNode = createNewUserNode(newPermissionRoot,
						userModel.getUser(), userPermissions,
						userDataPermissions, true);
				// mark user as new -> green
				if (!users.contains(userModel.getUser())) {
					((TreeNodeModel) currentUserNode.getData())
							.setMarking(Marking.NEW);
					((TreeNodeModel) newUserNode.getData())
							.setMarking(Marking.NEW);
				}
				...
				if (users.contains(userModel.getUser())) {
					...
					TreeNode currentUserNode = createCurrentUserNode(
							currentPermissionRoot, userModel.getUser(),
							userPermissions, userDataPermissions, false);
					TreeNode newUserNode = createNewUserNode(newPermissionRoot,
							userModel.getUser(), userPermissions,
							userDataPermissions, false);
					((TreeNodeModel) currentUserNode.getData())
	...
	private TreeNode createCurrentUserNode(TreeNode permissionRoot, User user,
			List<Permission> userPermissions,
			List<Permission> userDataPermissions, boolean addedUser) {
		TreeNodeModel userNodeModel = new TreeNodeModel(user.getUsername(),
				ResourceType.USER, user);
		DefaultTreeNode userNode = new DefaultTreeNode(userNodeModel,
				permissionRoot);
		userNode.setExpanded(true/* addedUser */);
		userNode.setSelectable(false);
		if (addedUser) {
			createCurrentPermissionTreeForAddedUser(userNode, userPermissions,
					userDataPermissions);
		} else {
			createCurrentPermissionTreeForRemovedUser(userNode,
					userPermissions, userDataPermissions);
		}
		return userNode;
	}

	private void createCurrentPermissionTreeForAddedUser(
			DefaultTreeNode userNode, List<Permission> userPermissions,
			List<Permission> userDataPermissions) {
		// added user receives new permissions from groups -> mark only
		// replaceable
		Set<Permission> replaced = permissionHandlingUtils
				.getReplacedByGranting(
						concat(userPermissions, userDataPermissions),
						permissionHandlingUtils.getGrantableFromSelected(
								userPermissions, selectedGroupPermissions).get(
								0));
		getPermissionTree(userNode, userPermissions, userDataPermissions,
				replaced, Marking.REMOVED);
	}

	private void createCurrentPermissionTreeForRemovedUser(
			DefaultTreeNode userNode, List<Permission> userPermissions,
			List<Permission> userDataPermissions) {
		...
		Set<Permission> revoked = permissionHandlingUtils
				.getRevokableFromRevoked(userPermissions,
						selectedGroupPermissions, true).get(0);
		getPermissionTree(userNode, userPermissions, userDataPermissions,
				revoked, Marking.REMOVED);
	}

	private TreeNode createNewUserNode(TreeNode permissionRoot, User user,
			List<Permission> userPermissions,
			List<Permission> userDataPermissions, boolean addedUser) {
		TreeNodeModel userNodeModel = new TreeNodeModel(user.getUsername(),
				ResourceType.USER, user);
	...
	private void createNewPermissionTreeForAddedUser(User user,
		...
		Set<Permission> replaced = permissionHandlingUtils
				.getReplacedByGranting(userPermissions, granted);
		...
		if (Boolean.valueOf(propertyDataAccess
			...
			getSelectablePermissionTree(userNode, new ArrayList<Permission>(
					currentPermissions), new ArrayList<Permission>(), granted,
					new HashSet<Permission>(), Marking.NEW, Marking.REMOVED);
		} else {
			getPermissionTree(userNode, userPermissions,
					new ArrayList<Permission>(), granted, Marking.NEW);
			selectedUserNodes = (TreeNode[]) ArrayUtils
					.addAll(selectedUserNodes,
							getSelectedNodes(userNode.getChildren()));
		}
	}
	...
	private void createNewPermissionTreeForRemovedUser(User user,
		...
		Set<Permission> granted = permissionHandlingUtils
				.getRevokableFromRevoked(userPermissions,
						selectedGroupPermissions, true).get(2);
		...
		if (!granted.isEmpty()) {
			for (Permission permission : selectedGroupPermissions) {
				if (!permissionHandlingUtils.contains(revoked, permission)
						&& permissionHandlingUtils.implies(revoked, permission)) {
		...
		currentPermissions = new HashSet<Permission>(
				permissionHandlingUtils
						.removeAll(currentPermissions, impliedBy));

		if (Boolean.valueOf(propertyDataAccess
				.getPropertyValue(Company.USER_LEVEL))) {
			getSelectablePermissionTree(userNode, new ArrayList<Permission>(
					currentPermissions), new ArrayList<Permission>(), granted,
					revoked, Marking.NEW, Marking.REMOVED);
		} else {
			List<Permission> currentUserPermissions = new ArrayList<Permission>(
					userPermissions);
			currentUserPermissions = (List<Permission>) permissionHandlingUtils
					.removeAll(currentUserPermissions, revoked);
			currentUserPermissions.addAll(granted);
			getPermissionTree(userNode, currentUserPermissions,
					new ArrayList<Permission>(), new HashSet<Permission>(
							currentUserPermissions), Marking.NONE);
		}
	}
	...
	public void rebuildCurrentPermissionTree() {
		for (TreeNode userNode : currentPermissionRoot.getChildren()) {
			...
			if (!userPermissions.isEmpty()) {
				if (Marking.NEW.equals(userNodeModel.getMarking())
						|| Marking.NONE.equals(userNodeModel.getMarking())) {
					// this is an added or an existing user

					Set<Permission> selectedPermissions = getSelectedPermissions(
							selectedUserNodes, userNode);
					Set<Permission> replaced = permissionHandlingUtils
							.getReplacedByGranting(userPermissions,
									selectedPermissions);
					...
					userNode.getChildren().clear();
					getPermissionTree(userNode, userPermissions,
							userDataPermissions, revoked, Marking.REMOVED);

				}
			} else {
				userNode.getChildren().clear();
				new DefaultTreeNode(new TreeNodeModel(
						"No permissions available", null, null), userNode)
						.setSelectable(false);
			}
		}
	}
	...
	public void confirmPermissions() {
		for (TreeNode userNode : newPermissionRoot.getChildren()) {
			...
			List<Permission> allPermissions = permissionManager
					.getPermissions(user);
			List<Permission> userPermissions = permissionHandlingUtils
					.filterPermissions(allPermissions).get(0);
			List<Permission> userDataPermissions = permissionHandlingUtils
					.filterPermissions(allPermissions).get(1);
			Set<Permission> selectedPermissions = getSelectedPermissions(
					selectedUserNodes, userNode);
			...
			if (Marking.NEW.equals(userNodeModel.getMarking())
				...
				for (Permission permission : replaced) {
					permissionService.revoke(userSession.getUser(), user,
							permission.getAction(), permission.getResource());
				}
				for (Permission permission : granted) {
					permissionService.grant(userSession.getUser(), user,
							permission.getAction(), permission.getResource());
				}
