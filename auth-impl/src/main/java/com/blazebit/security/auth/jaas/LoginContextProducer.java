package com.blazebit.security.auth.jaas;
...
import com.blazebit.security.auth.login.SimpleCallbackHandler;
...
public class LoginContextProducer implements Serializable {
	...
	@Produces
	public LoginContext produceLoginContext(
			@LoginModuleName final String loginModuleName)
			throws LoginException, URISyntaxException {
		if (loginContext == null) {
			synchronized (LOCK) {
				if (loginContext == null) {
					loginContext = createLoginContext(loginModuleName);
				}
			}
		}
		return loginContext;
	...
	public LoginContext createLoginContext(final String loginModuleName)
			throws LoginException, URISyntaxException {
		...
		try {
			return new LoginContext(loginModuleName, new Subject(),
					callbackHandler, new Configuration() {
						...
						public AppConfigurationEntry[] getAppConfigurationEntry(
							...
							entry[0] = new AppConfigurationEntry(
									loginModuleName,
