...
public class PermissionDataAccessImpl implements PermissionDataAccess {
    ...
    private static Set<Action> initExceptionalActions() {
        ...
        exceptionalActions.add(ActionUtils.getAction(ActionUtils.ActionConstants.GRANT));
        exceptionalActions.add(ActionUtils.getAction(ActionUtils.ActionConstants.REVOKE));
        exceptionalActions.add(ActionUtils.getAction(ActionUtils.ActionConstants.CREATE));
        exceptionalActions.add(ActionUtils.getAction(ActionUtils.ActionConstants.DELETE));
    ...
    public Set<Permission> getRevokablePermissionsWhenRevoking(Actor _actor, Action _action, Resource _resource) {
        ...
        if (permission != null) {
            ...
            baseQuery.append(" up.id.subject.id='").append(subject.getId()).append("' ");
    ...
    public boolean isGrantable(Actor subject, Action action, Resource _resource) {
        ...
        if (isExceptionalAction(action)) {
            if (!StringUtils.isEmpty(resource.getField())) {
                System.err.println("Exceptional action with wrong parameters");
                return false;
            }
            //entityid cannot be specified for add action
            if (action.matches(ActionUtils.getAction(ActionUtils.ActionConstants.CREATE))) {
    ...
    public Set<Permission> getRevokablePermissionsWhenGranting(Actor _actor, Action _action, Resource _resource) {
        ...
        query.append(" up.id.subject.id='").append(subject.getId()).append("' ");
    ...
    @Override
    public Set<Permission> findPermissionsFor(Actor _actor, String entityName, String entityId) {
        Set<Permission> ret = new HashSet<Permission>();
        IdHolder subject = (IdHolder) _actor;
        ...
        query.append(" and up.id.entity=''").append(entityName).append("'' ");

        if (StringUtils.isEmpty(entityId)) {
            ret.addAll(entityManager.createQuery(MessageFormat.format(query.toString(), getPermissionTypeFor(_actor.getClass()).getCanonicalName())).getResultList());
        } else {
            query.append(" and up.id.entityId=''").append(entityId).append("'' ");
            ret.addAll(entityManager.createQuery(MessageFormat.format(query.toString(), getDataPermissionTypeFor(_actor.getClass()).getCanonicalName())).getResultList());
        }
        return ret;
