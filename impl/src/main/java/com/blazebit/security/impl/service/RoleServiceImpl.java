...
public class RoleServiceImpl implements RoleService {
    ...
    public void addRoleToRole(Role role1, Role role2) {
        if (!role2.getRoles().contains(role1)) {
            ...
            throw new PermissionActionException("Role " + role1 + " cannot be added to role " + role2 + " because it is already added");
        }
    }
    ...
    public void removeRoleFromRole(Role role1, Role role2) {
        if (role2.getRoles().contains(role1)) {
            ...
            throw new PermissionActionException("Role " + role1 + " cannot be removed from role " + role2 + " because it doesn't contain it");
        }
    }
    ...
    public void addSubjectToRole(Subject authorizer, Subject subject, Role role, boolean copyPermissions) {
        ...
        if (!securityService.isGranted(authorizer, getGrantAction(), EntityUtils.getEntityObjectFieldFor(subject.getClass(), "", ((IdHolder) subject).getEntityId()))) {
            throw new PermissionException("Authorizer " + authorizer + " does not have grant action for " + subject);
        }
        ...
        if (!securityService.isGranted(authorizer, getGrantAction(), EntityUtils.getEntityObjectFieldFor(role.getClass(), "", ((IdHolder) role).getEntityId()))) {
            throw new PermissionException("Authorizer " + authorizer + " does not have grant action for " + role);
        }

        if (!canSubjectBeAddedToRole(subject, role)) {
            throw new PermissionActionException("Subject " + subject + " cannot be added to role " + role);
        }
        if (copyPermissions) {
    ...
    public void removeSubjectFromRole(Subject authorizer, Subject subject, Role role, boolean revokePermissions) {
        role = entityManager.find(role.getClass(), ((IdHolder) role).getId());
        if (!securityService.isGranted(authorizer, getRevokeAction(), EntityUtils.getEntityObjectFieldFor(subject.getClass(), "", ((IdHolder) subject).getEntityId()))) {
            throw new PermissionException("Authorizer " + authorizer + " does not have grant action for " + subject);
        }
        ...
        if (!canSubjectBeRemovedFromRole(subject, role)) {
            throw new PermissionActionException("Subject " + subject + " cannot be removed from role " + role);
        }
