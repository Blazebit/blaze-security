 ...
 */
@Stateless
public class PermissionServiceImpl extends PermissionCheckBase implements
		PermissionService {
	...
	@PostConstruct
	private void init() {
		actionImplicationProviders = ServiceUtils
				.loadServiceImplementations(ActionImplicationProvider.class);
	}
	...
	public boolean isGranted(Subject subject, Action action, Resource resource) {
		...
		return isGranted(permissions, action, resource,false);
	}
	...
	public boolean isGranted(Subject subject, Action action, Resource resource,
		...
		List<Permission> permissions = permissionManager
				.getPermissions(subject);
		return isGranted(permissions, action, resource, fieldLevel);
	}

	private boolean isGranted(List<Permission> permissions, Action action,
			Resource resource, boolean fieldLevel) {
		for (Permission permission : permissions) {
			if (permission.getAction().implies(action)
					&& permission.getResource().implies(resource)) {
				return true;
			}
		}
		...
		for (ActionImplicationProvider provider : actionImplicationProviders) {
			...
			for (Action implyAction : actions) {
				for (Permission permission : permissions) {
					if (permission.getAction().implies(implyAction)
							&& permission.getResource().implies(resource)) {
						return true;
					}
				}
	...
	@Override
	public void grant(Subject authorizer, Subject subject, Action action,
			Resource resource) throws PermissionException,
		...
		if (authorizer.equals(subject)) {
			throw new IllegalArgumentException(
					"Authorizer and subject cannot be the same");
		}
		checkParameters(subject, action, resource);
		if (!isGranted(authorizer, getGrantAction(),
				resourceFactory.createResource(subject))) {
			throw new PermissionException(authorizer
					+ " is not allowed to grant " + action + " to " + subject);
		}
		// TODO currently grant action to action not checked
		// if (!isGranted(authorizer, getGrantAction(),
		// entityFieldFactory.createResource(action))) {
		// throw new PermissionException(authorizer +
		// " is not allowed to grant " + getGrantAction() + " to " + action);
		// }
		if (!isGranted(authorizer, getGrantAction(), resource)) {
			throw new PermissionException(authorizer
					+ " is not allowed to grant " + resource);
		}
		if (!permissionDataAccess.isGrantable(subject, action, resource)) {
			throw new PermissionActionException("Permission for " + subject
					+ ", " + action + "," + resource + " cannot be granted");
		}
		Set<Permission> removablePermissions = permissionDataAccess
				.getRevokablePermissionsWhenGranting(subject, action, resource);
		for (Permission existingPermission : removablePermissions) {
			permissionManager.remove(existingPermission);
		}
		Permission permission = permissionFactory.create(subject, action,
				resource);
	...
	public void grant(Subject authorizer, Subject subject, Permission permission)
		...
		grant(authorizer, subject, permission.getAction(),
				permission.getResource());
	}

	@Override
	public void grant(Subject authorizer, Subject subject,
			Set<Permission> permissions) throws PermissionException,
			PermissionActionException {
		if (permissions == null) {
			throw new IllegalArgumentException("Permission cannot be null");
		}
	...
	@Override
	public void revoke(Subject authorizer, Subject subject, Action action,
			Resource resource) throws PermissionException,
			PermissionActionException {
	...
	@Override
	public void revoke(Subject authorizer, Subject subject, Action action,
			Resource resource, boolean force) throws PermissionException,
			PermissionActionException {
		if (authorizer == null) {
			throw new IllegalArgumentException("Authorizer cannot be null");
		}
		...
		if (!isGranted(authorizer, getRevokeAction(),
				resourceFactory.createResource(subject))) {
			throw new PermissionException(authorizer
					+ " is not allowed to revoke from " + subject);
		}
		// TODO currently revoke action to action not checked
		// if (!isGranted(authorizer, getRevokeAction(),
		// entityFieldFactory.createResource(getRevokeAction()))) {
		// throw new PermissionException(authorizer + " is not allowed to " +
		// getRevokeAction() + " to " + getRevokeAction());
		// }
		if (permissionDataAccess.isRevokable(subject, action, resource)) {
			Set<Permission> removablePermissions = permissionDataAccess
					.getRevokablePermissionsWhenRevoking(subject, action,
							resource);
			...
			// give it another try by revoking implying resources and granting
			// permissions
			if (force) {

				Set<Permission> toBeRevoked = new HashSet<Permission>();
				toBeRevoked.add(permissionFactory.create(subject, action,
						resource));
				List<Permission> currentPermissions = permissionManager
						.getPermissions(subject);

				Set<Permission> revoke = permissionHandling
						.getRevokableFromRevoked(currentPermissions,
								toBeRevoked, true).get(0);
				Set<Permission> grant = permissionHandling
						.getRevokableFromRevoked(currentPermissions,
								toBeRevoked, true).get(2);

				if (revoke.isEmpty()) {
					throw new PermissionActionException("Permission : "
				...
				throw new PermissionActionException("Permission : " + subject
						+ ", " + action + ", " + resource
						+ " cannot be revoked");
			}
		}
	...
	@Override
	public void revoke(Subject authorizer, Subject subject,
			Permission permission) throws PermissionException,
			PermissionActionException {
		if (permission == null) {
			throw new IllegalArgumentException("Permission cannot be null");
		}
		revoke(authorizer, subject, permission.getAction(),
				permission.getResource());
	}
	...
	@Override
	public void grant(Subject authorizer, Role role, Action action,
			Resource resource) throws PermissionException,
			PermissionActionException {
		if (authorizer == null) {
			throw new IllegalArgumentException("Authorizer cannot be null");
	...
	public void grant(Subject authorizer, Role role, Permission permission)
		...
		grant(authorizer, role, permission.getAction(),
				permission.getResource(), false);
	}
	...
	@Override
	public void grant(Subject authorizer, Role role, Action action,
			Resource resource, boolean propagate) throws PermissionException,
			PermissionActionException {
		if (authorizer == null) {
			throw new IllegalArgumentException("Authorizer cannot be null");
		}
		checkParameters(role, action, resource);
		if (!isGranted(authorizer, getGrantAction(),
				resourceFactory.createResource(role))) {
			throw new PermissionException(authorizer + " is not allowed to "
					+ getGrantAction() + " to " + resource);
		}
		// TODO currently grant action to action not checked
		// if (!isGranted(authorizer, getGrantAction(),
		// resourceFactory.createResource(getGrantAction()))) {
		// throw new PermissionException(authorizer +
		// " is not allowed to grant " + action + " to " + resource);
		// }
		if (!permissionDataAccess.isGrantable(role, action, resource)) {
			throw new PermissionActionException("Permission for " + role + ", "
					+ action + "," + resource + " cannot be granted");
		}

		Set<Permission> removablePermissions = permissionDataAccess
				.getRevokablePermissionsWhenGranting(role, action, resource);
		for (Permission existingPermission : removablePermissions) {
			permissionManager.remove(existingPermission);
		}
	...
	private void propagateGrantToSubjects(Subject authorizer, Role root,
		...
		for (Subject subject : subjects) {
			...
			toBeGranted
					.add(permissionFactory.create(subject, action, resource));
			List<Permission> currentPermissions = permissionManager
					.getPermissions(subject);
			// second: check if permission can be merged with existing ones
			Set<Permission> grant = permissionHandling.getGrantable(
					currentPermissions, toBeGranted).get(0);
			List<Set<Permission>> revokeAndGrant = permissionHandling
					.getRevokedAndGrantedAfterMerge(currentPermissions,
							new HashSet<Permission>(), grant);
	...
	@Override
	public void grant(Subject authorizer, Role role, Permission permission,
			boolean propagate) throws PermissionException,
			PermissionActionException {
		if (permission == null) {
			throw new IllegalArgumentException("Permission cannot be null");
		}
	...
	@Override
	public void grant(Subject authorizer, Role role,
			Set<Permission> permissions, boolean propagateToUsers)
			throws PermissionException, PermissionActionException {
		if (permissions == null) {
			throw new IllegalArgumentException("Permission cannot be null");
		}
	...
	public void revoke(Subject authorizer, Role role, Permission permission)
		...
		revoke(authorizer, role, permission.getAction(),
				permission.getResource(), false);
	}

	@Override
	public void revoke(Subject authorizer, Role role,
			Set<Permission> permissions) throws PermissionException,
			PermissionActionException {
		if (permissions == null) {
			throw new IllegalArgumentException("Permission cannot be null");
	...
	@Override
	public void revoke(Subject authorizer, Role role, Action action,
			Resource resource) throws PermissionException,
			PermissionActionException {
		if (authorizer == null) {
			throw new IllegalArgumentException("Authorizer cannot be null");
	...
	public void revoke(Subject authorizer, Role role, Permission permission,
		...
		revoke(authorizer, role, permission.getAction(),
				permission.getResource(), propagate);
	}

	@Override
	public void revoke(Subject authorizer, Role role,
			Set<Permission> permissions, boolean propagate)
			throws PermissionException, PermissionActionException {
		if (permissions == null) {
			throw new IllegalArgumentException("Permission cannot be null");
		}
	...
	@Override
	public void revoke(Subject authorizer, Role role, Action action,
			Resource resource, boolean force, boolean propagate)
			throws PermissionException, PermissionActionException {
		if (authorizer == null) {
			throw new IllegalArgumentException("Authorizer cannot be null");
		}
		...
		// TODO currently revoke action to action not checked
		// if (!isGranted(authorizer, getRevokeAction(),
		// resourceFactory.createResource(getRevokeAction()))) {
		// throw new PermissionException(authorizer + " is not allowed to " +
		// action + " to " + resource);
		// }
		...
		if (permissionDataAccess.isRevokable(role, action, resource)) {
			Set<Permission> removablePermissions = permissionDataAccess
					.getRevokablePermissionsWhenRevoking(role, action, resource);
			for (Permission permission : removablePermissions) {
				permissionManager.remove(permission);
			}
			permissionManager.flush();
			...
			if (force) {
				Set<Permission> toBeRevoked = new HashSet<Permission>();
				toBeRevoked.add(permissionFactory
						.create(role, action, resource));
				List<Permission> currentPermissions = permissionManager
						.getPermissions(role);
				Set<Permission> revoke = permissionHandling
				...
				if (!revoke.isEmpty()) {
					...
					throw new PermissionActionException("Permission : " + role
							+ ", " + action + ", " + resource
							+ " cannot be revoked");
				}
			} else {
	...
	private void propagateRevokeToSubjects(Subject authorizer, Role root,
			Action action, Resource resource, boolean force) {
		...
		for (Subject subject : subjects) {
			...
			// second: check if permission can be merged with existing ones
			Set<Permission> revoke = permissionHandling
					.getRevokableFromRevoked(currentPermissions, toBeRevoked,
							true).get(0);
			Set<Permission> grant = permissionHandling.getRevokableFromRevoked(
					currentPermissions, toBeRevoked, true).get(2);
			// merging might need revoke and grant operations
			revokeAndGrant(authorizer, subject, revoke, grant);
		}
		Collection<Role> children = root.getRoles();
	...
	@Override
	public void revokeAndGrant(Subject authorizer, Subject subject,
			Set<Permission> revoke, Set<Permission> grant) {
	...
	@Override
	public void revokeAndGrant(Subject authorizer, Role role,
			Set<Permission> revoke, Set<Permission> grant) {
		if (revoke == null || grant == null) {
			throw new IllegalArgumentException("Permissions cannot be null");
		}
	...
	@Override
	public void revokeAndGrant(Subject authorizer, Role role,
			Set<Permission> revoke, Set<Permission> grant, boolean propagate) {
		if (revoke == null || grant == null) {
			throw new IllegalArgumentException("Permissions cannot be null");
		}
		for (Permission permission : revoke) {
	...
	// @Override
	// public Collection<Action> getAllowedActions(Subject subject, Resource
	// resource) throws PermissionException,
	// PermissionActionException {
