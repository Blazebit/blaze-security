...
public class ResourcesBean extends ResourceGroupHandlingBaseBean {
    ...
    private void createUserNodes(User user, TreeNode currentRoot, TreeNode newRoot, Set<Permission> selectedPermissions) {
        ...
        buildNewPermissionTree(newUserNode, userPermissions, userDataPermissions, granted, new HashSet<Permission>(), replaced, !isEnabled(Features.FIELD_LEVEL),
    ...
    public void rebuildCurrentGroupPermissionTree() {
        for (TreeNode groupNode : currentGroupRoot.getChildren()) {
            ...
            rebuildCurrentTree(groupNode, permissions, selectedPermissions, new HashSet<Permission>(), groupReplaceables.get(userGroup), !isEnabled(Features.FIELD_LEVEL));
        }
    }
    ...
    private void createGroupNodes(UserGroup group, TreeNode currentRoot, TreeNode newRoot, Set<Permission> selectedPermissions) {
        ...
        List<Permission> groupPermissions = EntityPermissionUtils.getSeparatedPermissionsByResource(permissions).get(0);
        List<Permission> groupDataPermissions = EntityPermissionUtils.getSeparatedPermissionsByResource(permissions).get(1);
        ...
        buildCurrentPermissionTree(groupNode, groupPermissions, groupDataPermissions, new HashSet<Permission>(), replaced, !isEnabled(Features.FIELD_LEVEL));
        ...
        buildNewPermissionTree(newGroupNode, groupPermissions, groupDataPermissions, granted, new HashSet<Permission>(), replaced, !isEnabled(Features.FIELD_LEVEL), true, true);
