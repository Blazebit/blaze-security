...
public class PermissionDataAccessTest extends BaseTest<PermissionDataAccessTest> {
    ...
    private EntityResource emailEntityWithSubject;
    private EntityDataResource document2EntityTitleField;
    private EntityDataResource email1EntityTitleField;
    private EntityDataResource email1Entity;

    @BeforeDatabaseAware
    public void init() throws Exception {
        super.initData();
        email1Entity = (EntityDataResource) entityResourceFactory.createResource(Email.class, EntityResource.EMPTY_FIELD, 1);
        emailEntityWithSubject = (EntityResource) entityResourceFactory.createResource(Email.class, Subject_Field);
        document2EntityTitleField = (EntityDataResource) entityResourceFactory.createResource(Document.class, Title_Field, 2);
        email1EntityTitleField = (EntityDataResource) entityResourceFactory.createResource(Email.class, Subject_Field, 1);
     ...
     * @throws Exception
     */
    private Permission createPermission(Subject subject, Action action, Resource resource) {
     ...
     * @throws Exception
     */
    private Permission createPermission(Role role, Action action, Resource resource) {
        ...
        for (UserGroup userGroup : selectedGroups) {
            ...
            while (parent != null) {
                for (Permission p : permissionManager.getPermissions(parent)) {
                    ...
                    actualToBeRevoked.addAll(permissionDataAccess.getGrantImpliedPermissions(user, p.getAction(), p.getResource()));
                }
