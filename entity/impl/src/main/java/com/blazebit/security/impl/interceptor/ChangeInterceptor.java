	...
	public boolean onFlushDirty(Object entity, Serializable id,
		...
		for (String propertyName : changedPropertyNames) {
			isGranted = permissionService.isGranted(actionFactory
					.createAction(Action.UPDATE), resourceFactory
					.createResource(entity, propertyName));
			if (!isGranted) {
				break;
			}
		}
		...
		if (collection instanceof PersistentCollection) {
			...
			if (!oldValues.isEmpty()) {
				...
				isGrantedToRemove = permissionService.isGranted(actionFactory
						...
						resourceFactory.createResource(entity,
								fieldName));
			}
			newValues.removeAll(retained);
			if (!newValues.isEmpty()) {
				isGrantedToAdd = permissionService.isGranted(actionFactory
						.createAction(Action.ADD), resourceFactory
						.createResource(entity, fieldName));
			}
 ...
 */
	...
	public boolean onSave(Object entity, Serializable id, Object[] state,
				...
				if (state[i] != null) {
					...
					if (types[i].isAssociationType()) {
						...
						checkAssociation(entity, fieldName, types[i],
								Action.ADD, userContext,
								permissionService, resourceFactory,
								actionFactory);
					}
		...
		if (!foundNotEmptyField) {
			boolean isGranted = permissionService.isGranted(
					userContext.getUser(),
					actionFactory.createAction(Action.CREATE),
					resourceFactory.createResource(entity, id != null));
			if (!isGranted) {
				throw new PermissionActionException("Entity " + entity
						+ " is not permitted to be persisted by "
						+ userContext.getUser());
	...
	private void checkAssociation(Object entity, String propertyName,
			Type type, String action, UserContext userContext,
			PermissionService permissionService,
			EntityResourceFactory resourceFactory, ActionFactory actionFactory) {
	...
	public void onDelete(Object entity, Serializable id, Object[] state,
		...
		boolean isGranted = permissionService.isGranted(userContext.getUser(),
				actionFactory.createAction(Action.DELETE),
				resourceFactory.createResource(entity));
		if (!isGranted) {
